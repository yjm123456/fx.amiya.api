    ...
    public async Task<GoodsInfo> GetByIdAsync(string id)
    {
        var goodsInfo = await freeSql.Select<GoodsInfoDbModel>()
            ...
            GoodsInfo goods = new GoodsInfo()
            {
                ...
                CategoryName = string.Join(",", (freeSql.Select<GoodsCategoryDbModel>().Where(e => freeSql.Select<CategoryToGoodsDbModel>().Where(e => e.GoodsId == goodsInfo.Id).ToList(e => e.CategoryId).Contains(e.Id))).ToList().Select(e => e.Name).ToList()),

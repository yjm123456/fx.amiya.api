    ...
    public class IntegrationAccountRepository : IIntegrationAccountRepository
    {
        ...
        public async Task SaveIntegrationAccountAsync(IntegrationAccount entity)
        {
            ...
            if (integrationAccount == null)
            {
                ...
                await freeSql.Insert<IntegrationAccountDbModel>().AppendData(integrationAccountDbModel).ExecuteAffrowsAsync();
            }
            ...
            foreach (var record in entity.GenerateRecordList)
            {
                if (record.Id == null)
                {
                    ...
                    if (record.IntegrationRecord is ConsumptionIntegration consumptionIntegration)
                    {
                        ...
                        generateRecordId= await freeSql.Insert<IntegrationGenerateRecordDbModel>().AppendData(model).ExecuteIdentityAsync();
                    }
            ...
            if (entity.UseRecord != null)
            {
                ...
                integrationUseRecord.Id= await freeSql.Insert<IntegrationUseRecordDbModel>().AppendData(integrationUseRecord).ExecuteIdentityAsync();
                ...
                foreach (var item in entity.UseDetails)
                {
                    ...
                    await freeSql.Insert<IntegrationUseDetailRecordDbModel>().AppendData(integrationUseDetailRecord).ExecuteIdentityAsync();
                }

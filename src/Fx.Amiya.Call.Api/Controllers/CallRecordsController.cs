...
namespace Fx.Amiya.Call.Api.Controllers
{
    [Route("[controller]")]
    [ApiController]
    [FxInternalAuthorize]
    public class CallRecordsController : ControllerBase
    {
        ...
        public async Task<ResultData<IPageInfo<CallRecordVo>>> GetCallRecordsByEmployeeId(int? employeeId, DateTime startDate, DateTime endDate, int pageSize, int pageNum)
        {
            ...
            else
            {
                ...
                if (Convert.ToBoolean(employee.IsCustomerService) && position.IsDirector == false)
                {
                    filter = filterBuilder.And(filterBuilder.Eq("EmployeeID", employeeId) & filterBuilder.Gte("Date", startDate) & filterBuilder.Lt("Date", endDate));
                }
                else
        ...
        /// 根据通话编号集合获取通话记录列表
        /// </summary>
        /// <param name="callRecordIds"></param>
        /// <returns></returns>
        [HttpPost("getListByIds")]
        public async Task<ResultData<List<CallRecordVo>>> GetListByCallRecordIdsAsync(List<string> callRecordIds)
        {
            var filterBuilder = Builders<CallRecordVo>.Filter;
            FilterDefinition<CallRecordVo> filter = filterBuilder.Empty;
            filter = filterBuilder.In("ID", callRecordIds);
            var result = await repository.FindBySpecificationAsync(filter);
            return ResultData<List<CallRecordVo>>.Success().AddData("callRecord", result.ToList());
        }
